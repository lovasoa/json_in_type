...
fn test_nested_macro() {
    ...
    assert_eq!(
        r#"[{"ok":true}]"#,
        json_list![json_object! {ok:true}].to_json_string()
    );
    assert_eq!(
        r#"[{"a":0,"b":0}]"#,
        json_list![json_object! {a:0,b:0}].to_json_string()
    );
    assert_eq!(
        r#"[{"a":0,"b":0},{"a":0,"b":0}]"#,
        json_list![json_object! {a:0,b:0}, json_object! {a:0,b:0},].to_json_string()
    );
}
...
fn mem_size_example() {
    ...
    assert_eq!(
        r#"{"result_count":1,"errors":null,"results":[{"answer":42,"ok":true}]}"#,
